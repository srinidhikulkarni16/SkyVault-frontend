      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div
          ...
          className={`p-3 rounded-2xl transition-all duration-300 transform active:scale-90 shadow-sm ${
            isSelected 
              ? 'bg-lime-800 text-white shadow-lime-900/20' 
              : 'bg-stone-100 text-lime-700 hover:bg-lime-100'
          }`}
        >
          {isHovered || isSelected ? (
            <FolderOpen size={24} className="animate-in zoom-in duration-300" strokeWidth={2} />
          ) : (
          ...
          <div ref={menuRef} className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-2 z-50 min-w-[180px] bg-white border border-stone-200 rounded-2xl shadow-2xl py-2 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-50 hover:text-lime-800 transition-colors" onClick={action(() => onOpen(folder))}>
                  <FolderOpen size={14} /> Open Folder
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-50 hover:text-lime-800 transition-colors" onClick={action(onRename)}>
                  <Edit size={14} /> Rename
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-50 hover:text-lime-800 transition-colors" onClick={action(onMove)}>
                  <Move size={14} /> Move
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-50 hover:text-lime-800 transition-colors" onClick={action(onShare)}>
                ...
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-50 hover:text-lime-800 transition-colors" onClick={action(onStar)}>
                  <Star size={14} className={folder.is_starred ? 'text-amber-500 fill-amber-500' : ''} />
                  {folder.is_starred ? 'Unstar Folder' : 'Star Folder'}
                </button>
                <div className="my-1.5 border-t border-stone-100" />
                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 transition-colors" onClick={action(onDelete)}>
